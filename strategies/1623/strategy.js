 ...
 * the upper ATR channel (SMA + 2×ATR) on rising ATR — the channel
 * widening means volatility is expanding and a trend may be forming.
...
function onUpdate(ctx) {
    ...
    const atrAvg = ctx.avgVol ? null : null; // placeholder (ATR avg handled below)
    ...
    if (atr == null || atr_1 == null || sma20 == null || sma20_1 == null || close1 == null) {
        return null;
    }

    // ── ATR average (20-bar SMA of ATR) ─────────────────────────────────
    // Compute manually to detect ATR expansion/contraction
    let atrSum = 0;
    let atrCount = 0;
    for (let i = 0; i < 20; i++) {
    ...
    // 2. ATR is expanding (above its 20-bar average) — trend has conviction
    const atrExpanding = atrAvgVal != null && atr > atrAvgVal;
    // 3. Price above SMA (overall uptrend alignment)
    const aboveSMA = close1 > sma20;
    ...
    const atrContracting = atrAvgVal != null && atr < atrAvgVal;
