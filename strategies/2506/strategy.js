 ...
 * name: ETH 4H Volume-Surge Breakout Momentum
 * ex: binance
 * syms: ETHUSDT
 * interval: 4h
 * cash: 10000
 ...
 *   highest close since entry). Proven on BTC 4H across 3 walk-forward windows
 *   and on ETH/SOL on the recent window. This is a momentum family, the
 *   complement to the defensive band-bounce mean-reversion champion.
...
function onUpdate(ctx) {
  // 20-bar high lookback: the entry trigger. 20 is validated robust vs 10.
  let hh = -Infinity;
  for (let i = 1; i <= 20; i++) {
  ...
  if (pos > 0) {
    ...
    const s = ctx.state;
    // 3x ATR trail: validated as the robust exit across the flat 2.5-4x range.
