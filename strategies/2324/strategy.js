 ...
 * name: LTC-XRP-DOT Band Bounce 1D
 * ex: binance
 * syms: LTCUSDT, XRPUSDT, DOTUSDT
 * interval: 1d
 * cash: 1000
 *
 * Why this strategy: the Bollinger+RSI mean-reversion edge is confirmed on
 * mature low-volatility alts (LTC/XRP/DOT) at 4h. This tests the SAME logic on
 * the 1D timeframe — fewer, cleaner trades and lower fee drag. Bet: a liquid,
 * less volatile alt snaps back toward the middle band after touching the lower
 * band when RSI is oversold, but only in an uptrend.
 * When it buys: price touches the lower Bollinger band, RSI is oversold (<35),
 * and price is above the 200-SMA. When it sells: half at the middle band, the
 * rest on a trailing stop (price falls 8% from peak, or RSI turns overbought,
 * or a hard 12% stop below entry).
 * When it does NOT work: strong one-way downtrends (we stay in cash) and
 * choppy flat regimes where the 200-SMA gate whipsaws. It lags sustained bull
 * runs because it only buys on deep lower-band touches.
 */
