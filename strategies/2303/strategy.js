 ...
 * name: ETH Band Bounce
 * ex: binance
 * syms: ETHUSDT
 * interval: 4h
 * cash: 1000
 *
 * Bollinger Band mean reversion on a mature, liquid alt, gated by the 200-SMA
 * trend, with partial profit-taking so winners can run. This is the same logic
 * that validated on XRP and LTC (the family is asset-specific, so it must be
 * re-tested per asset). Bet: a liquid alt snaps back toward the middle band
 * after touching the lower band when RSI is oversold — but only in an uptrend.
 ...
 * runs because it only buys on deep lower-band touches. ETH's lower volatility
 * vs BTC but higher than LTC may change the edge — must be validated per asset.
 */
