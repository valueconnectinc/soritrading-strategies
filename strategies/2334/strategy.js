 ...
 * (price above the 200-period SMA). Sells the whole position at the middle band or when the
...
function onUpdate(ctx) {
  ...
  if (pos > 0) {
    // hard stop: cut a failed bounce at 12% below entry
    if (price <= ctx.entryPx * 0.88) {
      return { side: 'sell', qty: pos };
    }
    // exit the whole position once the bounce reaches the middle band or stoch recovers
