 ...
 * name: DOGE Regime Trend Champion 1D
 * ex: binance
 * syms: DOGEUSDT
 * interval: 1d
 * cash: 10000
 *
 * Why this strategy: The validated regime-trend recipe applied to a NEW asset (DOGE)
 * to test whether the edge generalizes beyond the majors it was proven on (ETH/BTC/
 * SOL/BNB/ADA/XRP). The bet: crypto trends persist for months, so riding price above
 * its 50-day average captures most of the upside, while a hysteresis exit and a crash
 * stop cut the worst drawdowns.
 * When it buys and sells: Buy when the last closed price is above the 50-day average,
 * sized down when ATR/price is high or the fear index is extreme (<=20). Sell when price
 ...
 * the hysteresis exit can whipsaw; it rides full drawdowns in grind-downs. Long-only.
 */
