 ...
 * beyond the 3-ATR crash band it exits fully to cash. Position is never allowed to
...
function onUpdate(ctx) {
  ...
  if (crashStop) {
    ...
    // Hard cap at 100% of equity: the raw formula can over-leverage when
    // volatility is low, which caused buy/sell churn and account destruction.
    targetQty = Math.min(targetValue / price, equity / price);
  }

  const curQty = pos;
  const diff = targetQty - curQty;
  // Only rebalance if the change is more than 0.5% of current size, to cut churn.
  if (Math.abs(diff) < 0.005 * Math.max(0.0001, curQty)) return null;
